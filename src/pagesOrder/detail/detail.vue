...
import type { List, OrderDetail } from '@/types/order'
import { onLoad, onReady } from '@dcloudio/uni-app'
import { ref } from 'vue'
import { OrderState, OrderStateMap } from '@/services/constants'
...
import { wxPayMiniPayAPI, wxPayMockAPI } from '@/services/payment'
...
const currentPage = pages.at(-1) as PageInstance
...
const { xtxGuessLike: guessRef, onScrollLower: onScrolltolower } = useGuessLike()
...
const onOrderPay = async () => {
  // 开发环境，模拟支付，后端只是修改订单状态
  if (import.meta.env.DEV) {
    await wxPayMockAPI(query.id)
  } else {
    // 生产环境，根据订单获取微信支付所需参数
    const res = await wxPayMiniPayAPI(query.id)
    // 调用微信支付
...
const onOrderConfirm = () => {
  uni.showModal({
    content: '为保证您的合法权益，请收到货确认无误后，再确认收货',
  ...
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        ...
        uni.redirectTo({
          url: '/pagesOrder/order-list/order-list',
        })
      }
    },
  })
...
const onOrderCancel = async () => {
  await putMemberOrderCancelByIdAPI(query.id, reason.value)
  popup!.value?.close?.()
