...
import type { OrderList, Item } from '@/types/order'
...
const getOrderListData = async () => {
  const res = await getMemberOrderAPI(requestParams)
...
const getMemberOrderData = () => {
  ...
  getMemberOrderAPI(requestParams)
    .then((res) => {
      orderList.value = [...orderList.value, ...res.result.items]
      if (+res.result.pages <= requestParams.pageNumber) {
        isFinish.value = true
      }
    })
    .catch(() => {
...
type OrderType = keyof typeof OrderStateMap
</script>
  ...
  >
    <view class="card" v-for="order in orderList" :key="order.id">
      ...
      <view class="status">
        ...
        <!-- 订单状态文字 -->
        <text>{{ OrderStateMap[order.orderState as OrderType].text }}</text>
