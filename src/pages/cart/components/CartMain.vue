...
const onDeleteCart = (skuId: string) => {
  uni.showModal({
    ...
    success: async (success) => {
      if (success.confirm) {
        // 后端删除商品
        await deleteMemberCartBySkuIdAPI([skuId])
...
const selectedCartListMoney = computed(() =>
  selectedList.value.reduce((total, item) => total + item.nowPrice * item.count, 0),
)
const selectedCartListCount = computed(() =>
  selectedList.value.reduce((total, item) => total + item.count, 0),
)
// 点击结算
const gotoPayment = () => {
...
<template>
  <scroll-view enable-back-to-top scroll-y class="scroll-view" @scrolltolower="onScrollLower">
    <!-- 已登录: 显示购物车 -->
    ...
    <!-- 猜你喜欢 -->
    <XtxGuess ref="xtxGuessLike" />
    <!-- 底部占位空盒子,正好可以放置toolbar，避免遮挡 -->
